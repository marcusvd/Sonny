...
export class ProductTypeGetMatSelectSingleComponent extends BaseForm implements OnInit {
  ...
  searchProductType() {
    this.productsTypes$ = this.productTypeFormControl.valueChanges.pipe(
      x => this.productsTypes$.pipe(
        map(xy => xy.filter(y => y.name.toLocaleLowerCase().includes(this.productTypeFormControl.value.toLocaleLowerCase()))))
    )
  }
