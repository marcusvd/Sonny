...
export class CollectDeliverAllListTableService extends BackEndService<CollectDeliverDto, number> {
  //Columns
  ...
  callBackEnd(pageIndex?: number, pageSize?: number, terms?: string, start?: Date, end?: Date) {
    ...
    ).subscribe((httpResponse: HttpResponse<CollectDeliverDto[]>) => {
      const HttpdataReturn = httpResponse;
      HttpdataReturn.body.forEach((element: CollectDeliverDto) => {

        console.log(element)

        const colDelTableDto = new CollectDeliveryTableDto();


        if (element?.destinyClientId)
          colDelTableDto.destiny = element?.destinyClient.name;

        if (element?.destinyCompanyId)
          colDelTableDto.destiny = element?.destinyCompany.name;

        if (element?.destinyPartnerId)
          colDelTableDto.destiny = element?.destinyPartner.name;

        if (element?.sourceClientId)
          colDelTableDto.source = element?.sourceClient.name;

        if (element?.sourceCompanyId)
          colDelTableDto.source = element?.sourceCompany.name;

        if (element?.sourcePartnerId)
          colDelTableDto.source = element?.sourcePartner.name;

        if (element?.destinyNoRegisterAddress || element?.destinyNoRegisterName)
          colDelTableDto.destiny = `${element?.destinyNoRegisterAddress} 'Nome: ' ${element?.destinyNoRegisterName} `

        if (element?.sourceNoRegisterAddress || element?.sourceNoRegisterName)
          colDelTableDto.source = `${element?.sourceNoRegisterAddress} 'Nome: ' ${element?.sourceNoRegisterName} `


       colDelTableDto.start = element.start;
        colDelTableDto.subject = element.subject;
        bodyReturnCollectDeliveryTableDto.push(colDelTableDto)
