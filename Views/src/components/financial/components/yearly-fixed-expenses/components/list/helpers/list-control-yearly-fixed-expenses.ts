...
export class ListControlYearlyFixedExpenses extends BaseList {

  entities$: Observable<ListYearlyFixedExpenseDto[]>;
  entities: ListYearlyFixedExpenseDto[] = [];
  entitiesFiltered$: Observable<ListYearlyFixedExpenseDto[]>;
  entitiesFiltered: ListYearlyFixedExpenseDto[] = [];
  ...
  onClickOrderByFields(field: string, entities$: Observable<ListYearlyFixedExpenseDto[]>) {
  ...
  supplyItemsGrid = (listFinancingsLoansExpenses: ListYearlyFixedExpenseDto[], entity: YearlyFixedExpenseDto) => {

    const items: ListYearlyFixedExpenseDto = new ListYearlyFixedExpenseDto();
  ...
  startSupply(url: string, cardId: string): Subscription {
    ...
    let entities: ListYearlyFixedExpenseDto[] = [];
  ...
  // isExpires(entities: ListYearlyFixedExpenseDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  //   if (selectedMonth == -1) {
  ...
  // isPending(entities: ListYearlyFixedExpenseDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  ...
  // isPaid(entities: ListYearlyFixedExpenseDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  //   if (selectedMonth == -1) {
  //     const result = entities.filter(x =>
  //       //check Year
  //       (this.currentDate.getFullYear() == new Date(x.expires.key).getFullYear())
