...
export class FrontEndFilterVariableExpenseslist {
  ...
  isExpires(entities: VariableExpensesListGridDto[], selectedMonth: number, currentPage: number, pageSize: number) {
    if (selectedMonth == -1) {
      const result = entities.filter(x =>
        //check Year
        (this.currentDate.getFullYear() == new Date(x.paidDay).getFullYear())
      ...
      return of(result.filter(x =>
        //chec.expiration
        (this.minValue.getFullYear() == new Date(x.paidDay).getFullYear())
        &&
        //checkIsExpires
        (new Date(this.currentDateWithoutHours) > new Date(new Date(x.paidDay).setHours(0, 0, 0, 0)))).slice(currentPage, pageSize))
    }
    else {
      ...
      const result = checkPeriod.filter(x =>
        ...
        &&
        //checkIsExpires
        (new Date(this.currentDateWithoutHours) > new Date(new Date(x.paidDay).setHours(0, 0, 0, 0)))
      )
  ...
  isPending(entities: VariableExpensesListGridDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  ...
  isPaid(entities: VariableExpensesListGridDto[], selectedMonth: number, currentPage: number, pageSize: number) {
    if (selectedMonth == -1) {
      const result = entities.filter(x =>
        //check Year
        (this.currentDate.getFullYear() == new Date(x.paidDay).getFullYear())
      ...
      return of(checkPeriod.filter(x => this.minValue.getFullYear() != new Date(x.paidDay).getFullYear()).slice(currentPage, pageSize))
    }
  }
