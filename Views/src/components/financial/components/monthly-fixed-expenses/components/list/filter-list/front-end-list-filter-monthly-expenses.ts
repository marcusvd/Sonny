...
//  override onSelectedMonth(entities: ListMonthlyFixedExpenseDto[], selectedMonth: number) {
//     let result = null;
...
//   getCurrentByCurrentYearAndEqualAndLessThenSelectedMonth(entities: ListMonthlyFixedExpenseDto[], currentPage: number, pageSize: number, selectedMonth: number) {
...
//   }
//   private applyFilterOnPageChangeMonthly(entities: ListMonthlyFixedExpenseDto[], month: number, startIndex: number, endIndex: number) {
//     if (month != -1) {
//       const result = entities.filter(x =>
//         this.currentDate.getFullYear() == new Date(x.expires).getFullYear()
//         && new Date(x.expires).getMonth() == month)
