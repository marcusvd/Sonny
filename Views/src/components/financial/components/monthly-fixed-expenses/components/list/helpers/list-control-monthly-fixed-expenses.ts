...
export class ListControlMonthlyFixedExpenses extends BaseList {

  entities$: Observable<ListMonthlyFixedExpenseDto[]>;
  entities: ListMonthlyFixedExpenseDto[] = [];
  entitiesFiltered$: Observable<ListMonthlyFixedExpenseDto[]>;
  entitiesFiltered: ListMonthlyFixedExpenseDto[] = [];
  ...
  onClickOrderByFields(field: string, entities$: Observable<ListMonthlyFixedExpenseDto[]>) {
  ...
  supplyItemsGrid = (listFinancingsLoansExpenses: ListMonthlyFixedExpenseDto[], entity: MonthlyFixedExpenseDto) => {

    const items: ListMonthlyFixedExpenseDto = new ListMonthlyFixedExpenseDto();
    ...
    Object.assign(items, {
      ...
      expiresView: {
        key: this._ptBrDatePipe.transform(entity.expires, 'Date'),
        styleCell: 'width:100%;',
  ...
  startSupply(url: string, cardId: string): Subscription {
    ...
    let entities: ListMonthlyFixedExpenseDto[] = [];
  ...
  // isExpires(entities: ListMonthlyFixedExpenseDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  //   if (selectedMonth == -1) {
  ...
  // isPending(entities: ListMonthlyFixedExpenseDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  ...
  // isPaid(entities: ListMonthlyFixedExpenseDto[], selectedMonth: number, currentPage: number, pageSize: number) {
  //   if (selectedMonth == -1) {
  //     const result = entities.filter(x =>
  //       //check Year
  //       (this.currentDate.getFullYear() == new Date(x.expires.key).getFullYear())
