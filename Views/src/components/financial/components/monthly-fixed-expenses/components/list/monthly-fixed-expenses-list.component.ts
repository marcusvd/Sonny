...
export class MonthlyFixedExpensesListComponent extends List implements OnInit {
 ...
 override addUrlRoute: string = '/side-nav/financial-dash/monthly-fixed-expenses-add';
  ...
  getData() {
    ...
    this.gridListCommonHelper.entitiesFromDbToMemory$.subscribe((x: MonthlyFixedExpensesDto[]) => {
      this.entities = [];
      x.forEach((xy: MonthlyFixedExpensesDto) => {
       console.log(xy)
        this.makeViewDto(xy);
      })
  ...
  makeViewDto(xy: MonthlyFixedExpensesDto) {
    console.log(xy)
