...
@Injectable()
export class ServicesResolver extends BackEndService<BudgetServiceDto> implements Resolve<Observable<{ budgetServiceLength: number }>>{
  ...
  resolve(
    ...
    const budgetServiceLength$: Observable<number> = this.loadById$('budgetsservices/LengthEditServicesAsync', route.paramMap.get('id'));
